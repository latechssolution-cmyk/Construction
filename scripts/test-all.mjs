/**
 * Comprehensive end-to-end test of every major module.
 * Run from project root: node scripts/test-all.mjs
 */
...
async function login() {
  ...
  // Only send the cookie that matches the token we're posting — sending both
  // authjs.csrf-token cookies causes MissingCSRF error on the server.
...
async function main() {
  ...
  console.log("  Construction ERP — Full System Test");
  console.log("=".repeat(60));
  ...
  const ledgerCountBefore = Array.isArray(ledger0) ? ledger0.length : 0;
  ...
  const ledgerCountAfterCreate = Array.isArray(ledger1) ? ledger1.length : 0;
  ...
  const matCreateEntry = Array.isArray(ledger1) && ledger1.find(e => e.description?.includes("TEST_CEMENT_AAA") && e.category === "material_purchase");
  ...
  check("Unit price updated after restock", mRestock?.unitPrice === 600);
  check("Total price cumulative", mRestock?.totalPrice === 110000, `got ${mRestock?.totalPrice} expected 110000`); // 50000 + 60000

  // Check ledger entry for restock
  const { data: ledger2 } = await get("/api/ledger");
  const ledgerCountAfterRestock = Array.isArray(ledger2) ? ledger2.length : 0;
  ...
  const restockEntry = Array.isArray(ledger2) && ledger2.find(e => e.description?.includes("TEST_CEMENT_AAA") && e.description?.includes("Restock"));
  ...
  const ledgerCountAfterEdit = Array.isArray(ledger3) ? ledger3.length : 0;
  ...
  check("All materials have id", Array.isArray(matList) && matList.every(m => m.id),
    `missing=${matList?.filter?.(m=>!m.id)?.length}`);
  check("All materials have project populated", Array.isArray(matList) && matList.some(m => m.project?.name));
  ...
  check("GET /api/employees → has id on all", Array.isArray(empList) && empList.every(e => e.id));
  ...
  // Mark invoice as paid — should create ledger entry
  const ledgerBeforePay = await get("/api/ledger");
  const ledgerBeforePayCount = Array.isArray(ledgerBeforePay.data) ? ledgerBeforePay.data.length : 0;

  const { status: invPaidSt, data: invPaid } = await put(`/api/invoices/${invId}`, {
    status: "paid", bankAccountId: bankId,
  });
  check("Mark invoice paid → 200", invPaidSt === 200, invPaid?.error);
  check("Invoice status is paid", invPaid?.status === "paid");
  check("paidAt timestamp set", !!invPaid?.paidAt);

  const ledgerAfterPay = await get("/api/ledger");
  const ledgerAfterPayCount = Array.isArray(ledgerAfterPay.data) ? ledgerAfterPay.data.length : 0;
  check("Ledger income entry on invoice paid", ledgerAfterPayCount > ledgerBeforePayCount,
    `before=${ledgerBeforePayCount} after=${ledgerAfterPayCount}`);
  const payEntry = Array.isArray(ledgerAfterPay.data) && ledgerAfterPay.data.find(e => e.category === "invoice_payment" && e.invoiceId === invId || e.referenceNumber === newInv?.invoiceNumber);
  check("Invoice payment ledger amount matches grandTotal", Math.round(payEntry?.amount) === Math.round(newInv?.grandTotal), `got ${payEntry?.amount}`);
  ...
  check("Ledger entries have id", Array.isArray(ledGet) && ledGet.every(e => e.id || e._id));
  ...
  const { status: eqPostSt, data: newEq } = await post("/api/equipment", {
    name: "TEST_EXCAVATOR", type: "heavy", model: "CAT 320", condition: "good", status: "available",
  });
  ...
  console.log("\n[CLEANUP]");
  // First: delete project-level dependents in parallel
  ...
  // Cancel project so client delete passes the active-project check, then delete project
  await put(`/api/projects/${projId}`, { status: "cancelled" });
  const wave2 = await Promise.all([
    del(`/api/projects/${projId}`),
  ]);
  // Finally delete the client and vendor (no more active projects/materials)
